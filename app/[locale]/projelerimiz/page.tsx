...
export async function generateMetadata({
  params,
}: PageProps<"/[locale]/projelerimiz">): Promise<Metadata> {
...
export default async function Page({
  params,
}: PageProps<"/[locale]/projelerimiz">) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const site = await getSiteContent(locale);
  const content = await getExpansionContent("projects", locale);
  return (
    <PortedFrame site={site} family="expansion" kind="projects">
      <ExpansionPage kind="projects" content={content} locale={locale} />
    </PortedFrame>
  );
}
