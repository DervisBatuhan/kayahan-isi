...
const KIND = "sofben" as const;
const PATH = "/sofben-servisi";

export async function generateMetadata({ params }: PageProps<"/[locale]/sofben-servisi">): Promise<Metadata> {
  const { locale } = await params;
  return buildPortedMetadata(locale, PATH);
}

export default async function Page({ params }: PageProps<"/[locale]/sofben-servisi">) {
  const { locale } = await params;
