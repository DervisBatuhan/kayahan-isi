...
const KIND = "gizlilik-politikasi";

export function generateStaticParams() {
  return locales.map((locale) => ({ locale }));
}

export async function generateMetadata({ params }: PageProps<"/[locale]/gizlilik-politikasi">): Promise<Metadata> {
  const { locale } = await params;
  return buildPortedMetadata(locale, "/gizlilik-politikasi");
}

export default async function Page({ params }: PageProps<"/[locale]/gizlilik-politikasi">) {
  const { locale } = await params;
