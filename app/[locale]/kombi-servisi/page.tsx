...
const KIND = "kombi" as const;
const PATH = "/kombi-servisi";

export async function generateMetadata({ params }: PageProps<"/[locale]/kombi-servisi">): Promise<Metadata> {
  const { locale } = await params;
  return buildPortedMetadata(locale, PATH);
}

export default async function Page({ params }: PageProps<"/[locale]/kombi-servisi">) {
  const { locale } = await params;
