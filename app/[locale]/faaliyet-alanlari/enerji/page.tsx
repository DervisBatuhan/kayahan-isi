...
const PATH = "/faaliyet-alanlari/enerji";

export async function generateMetadata({
  params,
}: PageProps<"/[locale]/faaliyet-alanlari/enerji">): Promise<Metadata> {
...
export default async function Page({
  params,
}: PageProps<"/[locale]/faaliyet-alanlari/enerji">) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const site = await getSiteContent(locale);
  const content = await getActivityContent("energy", locale);
  return (
    <PortedFrame site={site} family="activity" kind="energy">
      <ActivityPage kind="energy" content={content} locale={locale} />
    </PortedFrame>
  );
}
