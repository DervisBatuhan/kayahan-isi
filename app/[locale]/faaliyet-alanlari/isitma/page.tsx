...
const PATH = "/faaliyet-alanlari/isitma";

export async function generateMetadata({
  params,
}: PageProps<"/[locale]/faaliyet-alanlari/isitma">): Promise<Metadata> {
...
export default async function Page({
  params,
}: PageProps<"/[locale]/faaliyet-alanlari/isitma">) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const site = await getSiteContent(locale);
  const content = await getActivityContent("heating", locale);
  return (
    <PortedFrame site={site} family="activity" kind="heating">
      <ActivityPage kind="heating" content={content} locale={locale} />
    </PortedFrame>
  );
}
