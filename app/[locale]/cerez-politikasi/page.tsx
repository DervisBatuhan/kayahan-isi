...
export async function generateMetadata({ params }: PageProps<"/[locale]/cerez-politikasi">): Promise<Metadata> {
  ...
  return buildPortedMetadata(locale, "/cerez-politikasi");
}

export default async function Page({ params }: PageProps<"/[locale]/cerez-politikasi">) {
  const { locale } = await params;
