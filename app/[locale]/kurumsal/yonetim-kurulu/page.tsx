...
const PATH = "/kurumsal/yonetim-kurulu";

export async function generateMetadata({
  params,
}: PageProps<"/[locale]/kurumsal/yonetim-kurulu">): Promise<Metadata> {
...
export default async function Page({
  params,
}: PageProps<"/[locale]/kurumsal/yonetim-kurulu">) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const site = await getSiteContent(locale);
  const content = await getExpansionContent("board", locale);
  return (
    <PortedFrame site={site} family="expansion" kind="board">
      <ExpansionPage kind="board" content={content} locale={locale} />
    </PortedFrame>
  );
}
