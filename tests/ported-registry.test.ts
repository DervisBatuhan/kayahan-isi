...
describe("lib/content/ported/registry", () => {
  it("registers exactly 53 design pages (31 hubs+content + 3 service + 8 district + 4 brand + 4 legal + 3 utility)", () => {
    expect(PORTED_ENTRIES).toHaveLength(53);
  });

  it("covers the eleven families", () => {
    const families = new Set(PORTED_ENTRIES.map((e) => e.family));
    expect(families).toEqual(
      new Set(["hub", "corporate", "expansion", "activity", "solution", "knowledge", "service", "district", "brand", "legal", "utility"]),
    );
  });
